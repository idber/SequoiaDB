...
INT32 sfsSetxattr(const CHAR *path, const CHAR *name, const CHAR *value, size_t size, INT32 flags){return 0;}
INT32 sfsGetxattr(const CHAR *path, const CHAR *name, CHAR *value, size_t size){return 0;}
...
INT32 sfsReaddir(const CHAR *path, void *buf, fuse_fill_dir_t filler, off_t offset,
                 struct fuse_file_info *fi)
{
    SDB_LOB_VALID_CHECK();    
...
struct fuse_operations sfsFuseOper=
{
    ...
    sfsFgetattr,//Fgetattr  --only called after create, in some cases, if not implemented, call getattr after create will not get the lob, so called this instead
...
static INT32 sfsProcessArg(void *data, const CHAR *arg, INT32 key, struct fuse_args *outargs)
{
