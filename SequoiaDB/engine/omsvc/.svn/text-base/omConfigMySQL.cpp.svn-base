/*******************************************************************************
   ...
   Source File Name = omConfigMySQL.cpp
...
namespace engine
{
   ...
   INT32 OmMySQLNode::_init( const BSONObj& bsonNode,
   ...
   INT32 OmMySQLNode::_setServiceName( const string& serviceName )
   {
      INT32 rc = SDB_OK ;
   ...
   INT32 OmMySQLNode::_setDBPath( const string& dbPath, OmHost& host )
   {
      INT32 rc = SDB_OK ;
   ...
   OmMySQLConfTemplate::OmMySQLConfTemplate()
   {
   }
   ...
   OmMySQLConfProperties::OmMySQLConfProperties()
   {
   }
   ...
   bool OmMySQLConfProperties::isPrivateProperty( const string& name ) const
   {
   ...
   bool OmMySQLConfProperties::isAllPropertySet()
   {
   ...
   public:
      ...
      {
         if ( OM_BUSINESS_SEQUOIASQL_MYSQL !=
   ...
   public:
      int operator() ( const OmHost *host1, const OmHost *host2 )
      {
         ...
         INT32 roleNum1 = host1->count( _byMySQLBuz ) ;
         INT32 roleNum2 = host2->count( _byMySQLBuz ) ;
   ...
   private:
      byMySQLBuz _byMySQLBuz ;
   } ;

   class getBestDiskForMySQL
   {
   public:
      getBestDiskForMySQL( const OmHost &host ) : _host ( host )
      {
      }
   ...
   OmMySQLConfigBuilder::OmMySQLConfigBuilder(
   ...
   INT32 OmMySQLConfigBuilder::_build( BSONArray& nodeConfig )
   {
      INT32 rc = SDB_OK ;
...

      {
         ...
         {
            OmMySQLNode* node = dynamic_cast<OmMySQLNode*>( *iter ) ;
   ...
   void OmMySQLConfigBuilder::_setLocal()
   {
   ...
   INT32 OmMySQLConfigBuilder::_createNode()
   {
      INT32 rc = SDB_OK ;
      OmMySQLNode* node = NULL ;
      OmHost* host = NULL ;
      const simpleDiskInfo* disk = NULL ;
      string serviceName ;
      string dbPath ;

      host = _cluster.chooseHost( getBestHostForMySQL() ) ;
      ...
      disk = host->chooseDisk( getBestDiskForMySQL( *host ) ) ;
      if ( NULL == disk )
      {
         rc = SDB_SYS ;
   ...
   INT32 OmMySQLConfigBuilder::_getServiceName( const OmHost& host,
                                                   string& serviceName )
   {
      INT32 rc = SDB_OK ;
   ...
   INT32 OmMySQLConfigBuilder::_check( BSONObj& bsonConfig )
   {
      INT32 rc = SDB_OK ;
   ...
   INT32 OmMySQLConfigBuilder::_checkAndAddNode( const BSONObj& bsonNode )
   {
      INT32 rc = SDB_OK ;
      OmMySQLNode* node = NULL ;
      OmHost* host = NULL ;
   ...
   bool OmMySQLConfigBuilder::_isServiceNameUsed( const OmHost& host,
                                                     const string& serviceName )
   {
